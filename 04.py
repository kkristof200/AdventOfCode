...
def parse_passport(passport_str: s) -> dict:
    comps = [e.strip().split(':') for e in passport_str.split(' ')]

    return {e[0]:e[1] for e in comps}

def is_passport_valid_v1(passport: dict, needed_keys) -> bool:
...
def is_passport_valid_v2(passport: dict, needed_keys: list) -> bool:
    existing_keys = list(passport.keys())
...
passports = [p.strip().replace('\n', ' ') for p in s.split('\n\n')]
parsed_passports = [parse_passport(passport_str) for passport_str in passports]

# needed_keys = ['ecl', 'pid', 'eyr', 'hcl', 'byr', 'iyr', 'cid', 'hgt']
valid_passports = [p for p in parsed_passports if is_passport_valid_v2(p, needed_keys)]
